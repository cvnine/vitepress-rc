...
export function useRoute(fallbackComponent, ssrHref) {
    const [route, setRoute] = useState(getDefaultRoute);
    const latestPendingPathRef = useRef(null);
    useEffect(() => {
        function go(href = inBrowser ? window.location.href : ssrHref ? ssrHref : '/') {
