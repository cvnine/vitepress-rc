...
function makeRender(_code, defaultFn) {
    return [_code, '', 'render(', defaultFn, ')'].join('\n');
}
function isDestructing(tokens, start) {
...
async function transform({ code, local, scope }) {
    try {
        ...
        $(_code)
            ...
            let importName = item.match[2][0].value;
            (tmpImportMap[importName] || (tmpImportMap[importName] = [])).push(...node);
        });
        const entriesImports = Object.entries(tmpImportMap);
        if (local) {
            ...
            try {
                const r = await Promise.all(entriesImports.map((x) => {
                    let url;
        ...
        if (local) {
            ...
            for (const [key, val] of Object.entries(scope['css'])) {
                localImportCss.push(val.default);
            }
