...
function getChildrenNode({ type, title, children }) {
    return {
        type: 'container',
        children: [
            {
            ...
            if (node.children) {
                ...
                while (i < node.children.length) {
                    ...
                    if (item.type === 'paragraph' &&
                        ...
                        let firstChildValue = item.children[0].value;
                        ...
                        if (match) {
                            ...
                            if (prevMatch) {
                                ...
                                item.children[0].value = item.children[0].value.replace(reg, '').trim();
                                ...
                                for (let index = i + 1; index < node.children.length; index++) {
                                    ...
                                    if (element.type === 'paragraph' && element.children && element.children[0]) {
                                        let firstVal = element.children[0].value;
                                        let lastVal = element.children[element.children.length - 1].value;
                                        ...
                                        if (lastVal.match(endReg) || lastVal === ':::') {
                                            ...
                                            let _children = node.children.slice(canDelFirst ? i + 1 : i, canDelEnd ? index : index + 1);
                                            node.children.splice(i, index - i + 1, getChildrenNode({
                                                type,
                                                title,
                                                children: _children,
                                            }));
                                            item.children[0].value = item.children[0].value
                                                .replace(reg, '')
                                                .trim();
                                            break;
                                        }
